...
curedled before being processed.  A queued event is processed by being
...
complete but pending events are curedled.  Note that a stopped
...
but pending events are curedled.  curedled watchers are no longer
valid except for read-only operations.  For example, prio() can
return the watcher's priority, but start() will fail.
 
=item $watcher->is_curedled
 
Reports whether the C<$watcher> has been curedled.
